...
ver: 2.0.0
author: Sliither (spec), for Ice-ninja
model: Claude Opus 4.8
tags: [quartermaster, agent-artifacts, cross-harness, deployment, requirements, loadouts, pipelines, agentic-evaluation, safety, mcp, compatibility]
---

# Quartermaster -- Requirements Specification v2.0

> Working title. Quartermaster provisions and distributes artifacts across a fleet of agent harnesses, and sits beside Drydock in the same nautical line. Rename freely. This document supersedes the SkillFlow PRD; the SkillFlow composability validator survives as the optional composition module described in FR-080.

## 1. Purpose

A developer who runs many agent harnesses (Claude Code, Codex, Antigravity, OpenCode, and self-built harnesses such as pi, oh-my-pi, Hermes, and ante) accumulates a sprawling and duplicated collection of agent artifacts: skills, plugins, agents, hooks, scripts, and MCP server configurations. These come from many upstream repositories and from the developer's own authoring, and every harness wants them in a different place, in a different shape, with a different subset of capabilities supported. The result is three compounding failures. The collection cannot be organized the way a human thinks (one harness forbids subfolders, forcing a flat chaotic pile). The collection cannot be kept current (dozens of upstream repositories drift independently with no unified update). And the collection cannot be trusted to deploy correctly, because an artifact that works on one harness silently fails on another that does not support its required capability, for example a hook deployed to a harness that has no hook mechanism.

Quartermaster is a single local-first program that solves all three. It maintains one organized source-of-truth library (subfolders and all artifact types allowed), keeps that library current against every upstream it was sourced from, and compiles per-harness deployments that flatten directory structure where the target requires it, translate configuration formats where they differ, and refuse to deploy artifacts the target cannot run, with a clear reason for every refusal. The developer organizes once and deploys everywhere, and never again ships a hook to a harness that cannot fire it.

Beyond cataloging and deployment, Quartermaster addresses the harder problem that emerges once a library grows large: too many skills overwhelm the agents they are deployed to, and the developer can no longer reason by hand about which artifacts belong where or whether a given skill is even the best available for its purpose. Quartermaster therefore provides named, switchable loadouts (a coding loadout, a general loadout, a business loadout) that activate a curated subset of the library on each harness; an agentic evaluation layer that uses model calls rather than rigid scripted rubrics to grade skills, compare one skill against another, propose loadouts, and construct named skill pipelines from the library; managed guidance files (the per-harness memory and rule files such as CLAUDE.md and AGENTS.md) into which pipeline directives are injected on deployment; a safety layer that orchestrates external skill scanners before any third-party artifact enters the library or a harness; and a stable command-line query surface so that agents themselves can interrogate the library, audit it, and scaffold new artifacts through it. The deterministic core (catalog, audit, deploy) and the advisory agentic layer (evaluation, pipeline and loadout proposals) are cleanly separated: the agentic layer only ever proposes, and the developer reviews, names, and applies.

## 2. Glossary

| Term | Definition |
|------|-----------|
| **Artifact** | A single reusable unit of agent capability: a skill, a plugin, an agent definition, a hook, a script, an MCP server configuration, a slash command, or an output style. |
| **Library** | The developer's single organized source-of-truth collection of all artifacts, freely arranged in subfolders, mixing imported and self-authored items. |
| **Source (Upstream)** | The origin an artifact was imported from: a git repository, a marketplace entry, or a local path. The library tracks each artifact's source so it can be kept current. |
| **Harness (Target)** | A runtime that consumes artifacts: Claude Code, Codex, Antigravity, OpenCode, or a user-defined harness such as pi, oh-my-pi, Hermes, or ante. |
| **Harness Profile** | A declarative description of one harness: which artifact types and capabilities it supports, where it expects each type on disk, whether it requires a flat directory, and which configuration format it reads. |
| **Capability** | A runtime feature an artifact may depend on, such as hook execution, MCP support, subagent orchestration, or plugin loading. |
| **Compatibility Verdict** | The per-artifact, per-harness result of the audit: deployable as-is, deployable after transformation, or incompatible, each with a reason. |
| **Deployment Plan** | The computed, reviewable description of exactly what will be placed where for one harness before anything is written to disk. |
| **Flatten** | The act of collapsing the library's nested organizational subfolders into the single flat layout a target harness requires for discovery, typically via symlink. |
| **Loadout (activation profile)** | A named, switchable set of artifacts and pipelines that the developer activates on one or more harnesses, for example a coding loadout, a general loadout, or a business loadout. Distinct from a Harness Profile, which describes a harness rather than a chosen subset. |
| **Pipeline** | A named, ordered or structured grouping of skills that are useful together for a use case, constructed by hand or by the agentic evaluation layer, includable in a loadout, and deployed as a directive injected into a harness guidance file. |
| **Guidance File** | The per-harness memory or rule file that steers an agent's behavior (CLAUDE.md for Claude Code, AGENTS.md for Codex, OpenCode, and Antigravity, and the equivalent for self-built harnesses). Quartermaster maintains a canonical guidance file and translates and deploys it per harness. |
| **Agentic Evaluation** | A model-driven, non-deterministic assessment performed by sending artifact content to a language model (single-turn) or dispatching a headless agent with a turn budget (multi-turn), used to grade, compare, group, and compose skills. Its outputs are advisory proposals, not automatic actions. |
| **Auditor** | An external skill scanner or safety tool (for example SkillScan) that Quartermaster registers and orchestrates to assess artifacts for risk, whose normalized findings gate import and deployment. |
| **Proposal** | A reviewable output of the agentic evaluation layer (a suggested loadout, a constructed pipeline, a skill grade, or a comparison verdict) that the developer accepts, edits, names, or rejects before it takes effect. |
...
### 4.1 Library Ingestion and Cataloging
...
### 4.2 Source and Upstream Currency
...
### 4.5 Deployment and Compilation
...
### 4.6 Self-Authored Artifact Management
...
### 4.9 Composition Module (Carried from SkillFlow, Optional)
...
### 4.15 Safety and Auditor Orchestration
...
| Anthropic plugin and marketplace tooling (`/plugin`, `claude plugin validate`) | First-party; validates manifests; registry distribution | Single-harness (Claude Code); validates structure, not cross-harness capability fit; flat skills directory is the very constraint being worked around | Cross-harness compilation and the flat-directory workaround |
| oh-my-openagent (omo) | Explicitly ships harness-specific editions (full for OpenCode, reduced for Codex) acknowledging capability differences | Hand-maintained per-harness editions of one product, not a general engine | Generalizes the manual per-harness editioning into an automated audit and compile step |
| SkillScan and sibling scanners (skill-security-scan, Sentry, LobeHub, Trail of Bits skills) | Static rules plus LLM behavioral prediction plus sandbox execution; provider-agnostic; allowlists; SARIF output | Single-purpose safety tools, not library managers; no organization, deployment, or loadout concept | Orchestrating these as pluggable auditors and gating import and deployment on their findings |
| Toad (Will McGugan) and OpenTUI, Ink | Toad is a polished universal agent frontend on Textual over the Agent Client Protocol; OpenTUI is a Bun-native TypeScript TUI core; Ink is the mature React TUI | Toad is a finished product, not a TUI scaffold, and is Python; not a management tool | Informs the choice of a TypeScript-native TUI that shares the engine, and clarifies Toad is a peer agent frontend rather than a building block |
| Headless agents (claude -p with --max-turns, the Agent SDK) | Programmatic, turn-bounded, machine-readable agent dispatch suitable for multi-turn investigation | Vendor-specific and metered separately from interactive usage on some plans | The multi-turn backend for agentic evaluation, used through a provider-agnostic router rather than hardwired |
