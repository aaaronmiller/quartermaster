---
date: 2026-06-24 22:54:18 PDT
ver: 2.0.0
author: Sliither (design), for Ice-ninja
model: Claude Opus 4.8
tags: [quartermaster, design, bun, typescript, sqlite, opentui, svelte, loadouts, pipelines, agentic-evaluation, skillscan]
---

# Quartermaster -- Technical Design v2.0

## 1. Architecture Overview

Quartermaster is a local-first program structured as a thin set of surfaces (a command-line interface, a terminal user interface, a local web interface, and an agent query interface) over a pure domain core and a set of engines. The deterministic engines are a Catalog engine that turns a subfoldered, multi-source aggregated library into an indexed model, an Audit engine that computes per-artifact, per-harness compatibility verdicts from declarative harness profiles, a Deploy engine that compiles verdicts and the active loadout into a previewable, reversible deployment, a Loadout manager that activates curated subsets per harness, and a Guidance engine that maintains and translates per-harness rule files. Two further subsystems are model-driven and advisory: an Agentic Evaluation engine that grades, compares, and proposes loadouts and pipelines, and an Auditor orchestrator that drives external safety scanners. Harness knowledge lives entirely in profile data, not code. The library is read-only to the engines; all mutation happens in the targets and is recorded for reversal. The deterministic core never depends on the agentic layer; the agentic layer only ever produces proposals the developer accepts.
...
| Git access | System git via subprocess | Avoids a heavy bundled dependency; degrades gracefully when git is unavailable, matching the prefer-graceful-degradation principle |
...
| Agentic router | OpenAI-compatible endpoint (developer's model gateway) plus optional headless agent (`claude -p --max-turns`, Agent SDK) | Provider-agnostic, cost-routable, and decoupled from any single subscription given the interactive-versus-programmatic metering split |
| Safety scanners | External auditors orchestrated as subprocesses (SkillScan and siblings) | A mature category after the early-2026 supply-chain attacks; orchestrate rather than rebuild; provider-agnostic LLM prediction fits the gateway |
| TUI | OpenTUI (Bun-native TypeScript, Zig core) | Shares the engine in-process; production-proven in OpenCode; avoids a separate Python or Go runtime; Ink is the conservative fallback |
| Web interface | Svelte 5 + Hono + shadcn-svelte, dark-mode first, built with frontend-design-masterclass | Matches the existing stack; served locally by Bun and Hono; optionally packaged as a Tauri 2 desktop app |
...
- Context: This is the direct answer to the sourcing question. Three models were possible. (A) Symlink harness directories directly from the existing local clones in `/git/<project>`, treating each clone as the live source. (B) Clone fresh copies from the remote into a Quartermaster-managed location, duplicating repositories already present locally. (C) Maintain one aggregated managed library that is the source of truth for deployment, populated by sync from upstreams, where each artifact's source record carries both a canonical remote origin and an optional local mirror path that sync prefers when present.
- Options considered: A, B, and C above.
- Chosen because (C): the entire premise of the project is that the developer imposes their own subfolder taxonomy, which is impossible under model A because the layout is dictated by however each upstream repository happens to organize itself, and the library would be a fragile federation of many scattered repositories where moving or deleting a clone breaks links. Model B wastes effort re-cloning what already exists and creates a second copy that drifts. Model C gives one place to organize, back up, version, and reason about the curated collection, lets self-authored and imported artifacts live side by side, decouples the developer's organization from upstream restructuring, and makes deployment a clean library-to-harness operation. Because artifacts are tiny text files, the storage cost of a curated copy is negligible, which removes the only real argument for model A. The honest reconciliation with the instinct to target existing versions is the local-mirror preference: when a `/git/<project>` clone already exists, sync reads from it rather than hitting the network, so Quartermaster targets the existing version while still recording the remote origin so the library works on a fresh machine and currency checks can reach upstream.
- Trade-offs accepted: an edit made directly inside a `/git` clone is not live in the harness until a sync brings it into the library; this is acceptable because sync of text files is near-instant, and for the specific case where a developer wants a subtree linked live to a working clone, a source may be registered as a linked subtree rather than a synced copy. The default remains the synced aggregated library.

**Decision: Agentic evaluation is provider-agnostic, advisory, and split into single-turn and multi-turn modes**
- Context: The developer wants model-driven, non-scripted assessment of skills and model-constructed loadouts and pipelines, with configurable turn counts, and runs a model gateway routing several providers. Programmatic agent usage is also metered separately from interactive usage on some plans, so binding to one vendor's subscription is unwise.
...
- Chosen because: a deterministic rubric cannot judge natural-language programs well, which is the developer's stated reason for wanting agentic evaluation; a single-vendor binding is fragile under the metering split and the developer's own multi-provider routing. A router with a cheap single-turn path for bulk grading and a turn-bounded multi-turn path (a headless agent that can read files) for deep pipeline construction fits both the cost reality and the depth requirement.
...
- Context: The developer wants both a TUI and a clean dark-mode web interface, asked about Toad and Bubble Tea, and has a Bun and TypeScript core. Toad is a finished universal agent frontend built on Textual in Python over the Agent Client Protocol; it is a peer to the harnesses, not a scaffold to build on.
...
- Chosen because: a Python, Go, or Rust TUI would fragment the stack and force a separate process or foreign-function bridge; OpenTUI is Bun-native TypeScript, powers OpenCode in production, and lets the TUI share the engine directly. The web interface uses the developer's existing Svelte, Hono, and shadcn stack and is built with the frontend-design-masterclass skill, dark-mode first.
...
**Note on hosting:** Quartermaster is a local-first tool. The web interface is served by a local Bun and Hono process over a localhost API, optionally packaged as a desktop app; there is no remote service to host, so the usual Cloudflare and Vercel targets do not apply to the core. Distribution is a published package and a compiled single binary.
...
Harness profiles are not stored in SQLite; they live as YAML files (built-in under the package, user profiles under the config directory) so they are shareable and version-controllable. The registry loads and validates them at startup and may cache a parsed copy.
...
### 4.1 Catalog engine (`src/catalog`)
...
- Interface: `scan(roots): ScanResult`, `search(query): Artifact[]`, `rescanIncremental(): ChangeSet`.
...
### 4.2 Capability inference (`src/catalog/capabilities`)
...
### 4.3 Profile registry (`src/profiles`)
...
- Interface: `verdict(artifact, profile): Verdict`, `matrix(artifacts, profiles): Verdict[][]`.
...
### 4.6 Adapters (`src/deploy/adapters`)
...
### 4.7 Sources and sync (`src/sources`)
...
### 4.9 Composition module (`src/compose`, optional)
...
### 4.10 Loadout manager (`src/loadouts`)
...
### 4.11 Pipeline engine (`src/pipelines`)
...
### 4.12 Guidance engine (`src/guidance`)
...
### 4.13 Agentic Evaluation engine (`src/eval`)
...
- Interface: `grade(artifactIds, categories): Proposal`, `compare(artifactIds, categories): Proposal`, `proposeLoadouts(): Proposal[]`, `constructPipelines(instruction?, maxTurns): Proposal[]`.
...
interface ModelRouter {
  single(req: { system: string; prompt: string; model?: string }): Promise<string>;        // OpenAI-compatible
  multiTurn(req: { task: string; tools: "read-library"; maxTurns: number; model?: string }): Promise<string>;
}
```

### 4.14 Auditor orchestrator (`src/audit-safety`)
- Responsibility: register external auditors, invoke them on import and pre-deploy, normalize their output into `audit_findings`, gate deployment on a configurable threshold, and honor the trusted allowlist. Subsumes and extends the static risk scanner of 4.8.
...
### 4.15 Agent query interface (`src/query`) and surfaces (`src/cli`, `src/tui`, `src/web`)
...
qm import <source>
  Import artifacts from owner/repo, a repo URL, a repo subdir, a marketplace id, or a local path.

qm sync [<source>] [--check]
  Check or update upstreams. --check reports without writing. Never overwrites locally modified artifacts without confirmation.

qm audit [--harness ID] [--type T] [--only blocked|transform]
  Print the compatibility matrix or a filtered slice with reasons.

qm plan <harness|group|all> [--scope PROFILE|--tag T|--path SUBTREE]
  Dry-run: show every placement, method, transformation, and skip with reason. No disk writes.

qm deploy <harness|group|all> [--scope ...] [--yes]
  Apply a plan. Shows the plan and waits unless --yes. Records the deployment for reversal.

qm revert <deployment-id | --last>
  Reverse a recorded deployment to its prior state.

qm status [<harness>]
  Show deployed artifacts, method, in-sync or drifted, and orphaned deployments.

qm profile list | show <id> | new <id> | edit <id> | validate <id>
  Manage harness profiles.

qm new <type> <org-path>
  Scaffold a self-authored artifact of the given type at the given library subfolder.

qm compose validate <composition.yaml>
  Optional: validate a composition with the Noun/Verb/Adjective model.

qm loadout list | new <name> | add <name> <artifact|pipeline> | assign <name> <harness>
            | switch <harness> <name> | copy <name> <from-harness> <to-harness>
  Manage named loadouts and their assignment and activation per harness.

qm pipeline list | new <name> | validate <name> | add-to <loadout> <name>
  Define, validate, and attach pipelines.
...
qm evaluate build-pipelines [--instruction "..."] [--max-turns N] [--model M]
  Run advisory agentic evaluation. Results become proposals.

qm proposal list | show <id> | accept <id> | reject <id>
  Review and act on agentic proposals.

qm guidance edit | deploy <harness|all>
  Edit the canonical guidance file; deploy translated per-harness files with active pipeline directives.

qm safety register <auditor> | scan <artifact|--all> | threshold <score>
  Register and run safety auditors; set the deployment gate.

qm query list | search <q> | describe <artifact> | audit <artifact> | scaffold <type> --json
  Stable, machine-readable surface for agents. Mirrored by an optional MCP server.

qm tui            # launch the terminal interface (OpenTUI)
qm web [--port N] # serve the local dark-mode web interface
```

### 5.2 Harness profile contract
...
layout:
  skill:
    ...
    dirname: skills          # plural
    flat:    true            # only direct children are discovered, so flatten
  mcp:
...
layout:
  skill:
    ...
    dirname: skill           # singular, differs from Claude Code
    flat:    true
  mcp:
...
### 5.3 Config translation contract
...
### 6.4 Supply chain
The tool's own dependency surface is kept minimal (Bun built-ins, a small parser set) and lockfile-pinned. Artifact-level supply-chain risk (the dominant attack surface, demonstrated by the early-2026 campaigns against the skill ecosystem) is handled by the Auditor orchestrator, which gates import and deployment on registered scanner findings and a configurable threshold.
...
4. iamzhihuix/skills-manage and xingkongliang/skills-manager: local-first, multi-tool, presets and collections as prior art for management ergonomics.
...
7. SkillScan (NMitchem/SkillScan) and sibling scanners: three-stage static, LLM-prediction, and sandbox auditing, provider-agnostic, SARIF output, allowlists; the model for the Auditor orchestrator.
8. Headless agent dispatch: `claude -p` with `--max-turns` and structured output, and the Agent SDK; the multi-turn evaluation backend, used through a provider-agnostic router given the interactive-versus-programmatic metering split.
9. OpenTUI (anomalyco/opentui): Bun-native TypeScript TUI core powering OpenCode; the chosen terminal surface. Toad (Will McGugan) and Textual noted as a peer Python agent frontend, not a scaffold.
10. Guidance-file convention: CLAUDE.md for Claude Code and the cross-tool AGENTS.md standard read by Codex, OpenCode, and Antigravity, informing the Guidance engine's per-harness translation.
